...
export default function WhyRobusta({ mode }) {
    ...
    if (mode === "deep" && !played.current) {
      ...
      cards.forEach((card, index) => {
        ...
        tl.to(movingTrack.current, {
          y: () =>
            -(card.offsetTop - window.innerHeight / 2 + (isMobile ? 60 : 110)),
          duration: 1.2,
          ease: "expo.inOut",
        })
          .to(box, {
        ...
        if (dotGroups[index]) {
          tl.to(
            dotGroups[index].querySelectorAll(".dot"),
            {
  ...
  useEffect(() => {
    if (!isIntroActive) {
      const videos = gsap.utils.toArray(".level-video video");

      const triggers = videos.map((video) =>
  ...
  return (
    <div
      ref={containerRef}
      className={`game-intro-viewport ${
        !isIntroActive ? "intro-complete" : ""
      }`}
    >
      {/* Background Video */}
